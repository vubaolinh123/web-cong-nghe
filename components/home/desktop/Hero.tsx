...
export default function Hero() {
  ...
  return (
    <section className="relative h-full min-h-screen flex items-center justify-center overflow-x-clip bg-slate-950">
      {/* Optimized Background Effects - Reduced on mobile */}
      <div className="absolute inset-0">
        {/* Gradient Orbs - Smaller on mobile */}
        <div className={`absolute top-1/4 left-1/4 ${isMobile ? 'w-32 h-32 opacity-20' : 'w-96 h-96'} bg-blue-500/30 rounded-full blur-3xl animate-orb-pulse`} />
        <div className={`absolute bottom-1/4 right-1/4 ${isMobile ? 'w-32 h-32 opacity-20' : 'w-96 h-96'} bg-cyan-500/20 rounded-full blur-3xl animate-orb-pulse-delayed`} />
        {!isMobile && (
      ...
      <Container className="relative z-10">
        <div className={`grid grid-cols-1 ${isDesktop ? 'lg:grid-cols-2 gap-8 lg:gap-12' : 'gap-4'} items-center ${isDesktop ? 'min-h-[calc(100vh-100px)]' : 'py-16 sm:py-24'}`}>
          ...
          <div className={`text-center ${isDesktop ? 'lg:text-left order-2 lg:order-1' : ''}`}>
            ...
            <motion.h1
              ...
              className="text-3xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold text-white leading-tight mb-4 sm:mb-6"
            >
      ...
      {/* Right: 3D Scene - Absolute positioned to span full right side */}
      {isDesktop && (
        <motion.div
